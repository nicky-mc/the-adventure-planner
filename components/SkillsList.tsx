    ...
    <div className="h-full flex flex-col bg-[#F5F5DC] p-2">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
            {filteredSkills.map(skill => {
                ...
                return (
                    <div key={skill.name} className="grid grid-cols-12 items-center text-xs font-merriweather border-b border-stone-300 py-1 hover:bg-white/50 transition-colors">
                        <div className="col-span-1 flex justify-center">
                            <input 
                                type="checkbox" 
                                checked={skill.isClassSkill} 
                                onChange={(e) => updateSkill(skill.name, { isClassSkill: e.target.checked })}
                                className="accent-stone-800"
                            />
                        </div>
                        <div className="col-span-4 font-bold text-stone-800 pl-1 truncate" title={skill.name}>
                            {skill.name} <span className="text-[9px] font-normal text-stone-500">({skill.attribute})</span>
                        </div>
                        ...
                        <div className="col-span-2 text-center">
                             <input 
                                ...
                                value={skill.ranks}
                                onChange={(e) => updateSkill(skill.name, { ranks: parseInt(e.target.value) || 0 })}
                             />
                        </div>
                        <div className="col-span-2 text-center">
                             <input 
                                ...
                                value={skill.miscMod}
                                onChange={(e) => updateSkill(skill.name, { miscMod: parseInt(e.target.value) || 0 })}
                             />
                        </div>
                    </div>
                );
